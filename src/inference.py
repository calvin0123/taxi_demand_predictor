...
def load_batch_of_features_from_store(
    ...
    # data processing to convert timestamp to datetime[ns, UTC]
    fetch_data_from = pd.to_datetime(fetch_data_from, utc=True)
    fetch_data_to = pd.to_datetime(fetch_data_to, utc=True)
    ts_data = ts_data[ts_data.pickup_hour.between(fetch_data_from, fetch_data_to)]

    # Note: in the Pau file/data, there is timestamp, but in our case there is not timestamp so we comment out these codes
    # pickup_ts_from = int(fetch_data_from.timestamp() * 1000) # ts_data, not in our script
...
def load_model_from_registry():
    ...
    model = model_registry.get_model(
        ...
        # name=config.MODEL_NAME_XGBOOST,
        version=2, # xgboost version == 2
    )  
    
    model_dir = model.download()
    # model = joblib.load(Path(model_dir)  / 'model.pkl')
    model = joblib.load(Path(model_dir)  / 'xgboost_model.pkl')
